/**
 * @file Tests for ingestTurn() - completion appending, system
 *       exclusion default, includeRoles opt-in, scope passthrough.
 */
...
test('forwards scope unchanged', async () => {
  const { client, ingestCalls } = makeFakeClient();
  const customScope = { user: 'pip', namespace: 'mastra-app' };
  await ingestTurn(client, {
    messages: [{ role: 'user', content: 'q' }],
    completion: 'a',
    scope: customScope,
